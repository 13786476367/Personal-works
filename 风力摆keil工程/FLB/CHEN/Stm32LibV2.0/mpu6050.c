...
static bool check_acc=false,check_gyro=false;	//三轴重力加速度和角速度校验标志位


static void mpu6050_offest(MPU_6AXIS *const check);
...
void Mpu6050_CheclAcc(void)//校验三轴重力加速度
{
	check_acc = true;
}
void Mpu6050_CheckGyro(void)//校验三轴角速度
{
	check_gyro = true;
}
...
static void mpu6050_offest(MPU_6AXIS *const check)
{
	#define CHEKC_COUNT 200
	static MPU_6AXIS offset;
	if(check_acc)//校验acc
	{
		static uint8_t cnt_acc=0;
		static int32_t tempax=0,tempay=0,tempaz=0;
		if(cnt_acc==0)//初始化角速度零漂值（200次采样平均值）
		{
			offset.acc.x=0;
			offset.acc.y=0;
			offset.acc.z=0;
		...
		if(++cnt_acc==CHEKC_COUNT)
		{
			check_acc = false;
			offset.acc.x = tempax/cnt_acc;
			offset.acc.y = tempay/cnt_acc;
			offset.acc.z = tempaz/cnt_acc;
			cnt_acc = 0;
		}
	}
	if(check_gyro)//校验gyro
	{
		static uint8_t cnt_gyro=0;
		...
		if(++cnt_gyro==CHEKC_COUNT)
		{
			check_gyro = false;
			offset.gyro.x = tempgx/cnt_gyro;
			offset.gyro.y = tempgy/cnt_gyro;
			offset.gyro.z = tempgz/cnt_gyro;
			cnt_gyro = 0;
		}
	}
	
	check->acc.x -= offset.acc.x; 
	check->acc.y -= offset.acc.y;
	check->acc.z -= offset.acc.z;
	check->gyro.x -= offset.gyro.x;
	check->gyro.y -= offset.gyro.y;
	check->gyro.z -= offset.gyro.z;
